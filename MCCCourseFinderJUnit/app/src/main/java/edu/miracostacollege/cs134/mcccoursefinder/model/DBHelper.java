...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void addCourse(Course course) {
        ...
        long id = db.insert(COURSES_TABLE, null, values);
        course.setId(id) ;
    ...
    public int deleteCourse(Course course) {
        ...
        int success = db.delete(COURSES_TABLE, COURSES_KEY_FIELD_ID + " = ?",
    ...
    public int deleteAllCourses() {
        if(getAllCourses().size() < 1)
            return 0 ;

        SQLiteDatabase db = this.getWritableDatabase();
        int deleted = db.delete(COURSES_TABLE, null, null);
        db.close();
    ...
    public void addInstructor(Instructor instructor) {
        ...
        long id = db.insert(INSTRUCTORS_TABLE, null, values);
        instructor.setId(id);
    ...
    public int deleteInstructor(Instructor instructor) {
        ...
        // DELETE THE TABLE ROW
        int success =db.delete(INSTRUCTORS_TABLE, INSTRUCTORS_KEY_FIELD_ID + " = ?",
    ...
    public int deleteAllInstructors() {
        if(getAllInstructors().size() < 1)
            return 0 ;

        SQLiteDatabase db = this.getWritableDatabase();
        int deleted = db.delete(INSTRUCTORS_TABLE, null, null);
        db.close();
        return deleted ;
    }
    ...
    public void addOffering(int crn, String semester, long courseId, long instructorId) {
        ...
        values.put(FIELD_SEMESTER, semester);
    ...
    public int deleteOffering(Offering offering) {
        if(offering == null)
            return 0 ;
        ...
        // DELETE THE TABLE ROW
        int success = db.delete(OFFERINGS_TABLE, FIELD_CRN + " = ?",
                new String[]{String.valueOf(offering.getCRN())});
        db.close();
        return success ;
    }

    public int deleteAllOfferings() {
        if(getAllOfferings().size() < 1)
            return 0 ;

        SQLiteDatabase db = this.getWritableDatabase();
        int deleted = db.delete(OFFERINGS_TABLE, null, null);
        db.close();
    ...
    public Offering getOffering(int crn) {
        ...
        Offering offering = new Offering(cursor.getInt(0),
                cursor.getString(1), course, instructor);
    ...
    public boolean importOfferingsFromCSV(String csvFileName) {
        ...
        try {
            while ((line = buffer.readLine()) != null) {
                ...
                addOffering(crn, semester, courseId, instructorId);
            }
        } catch (IOException e) {
            e.printStackTrace();
            return false;
